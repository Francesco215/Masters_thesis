...
\begin{figure}[h]
    ...
    \caption{On the left there is the lattice structure of graphene made out of two interpenetrating triangular lattices ($a_1$ and $a_2$ are the lattice unit vectors, and $\mathbf \delta_{i}$ with $i\in {1,2,3}$ are the nearest neighbors. On the right the corresponding Brillouin zone, the Dirac cones are located at the $K_0$ and $K_1$ points). Image taken from \cite{guinea2008review}.}
    \label{fig:graphene}
\end{figure}\\
...
\end{equation}
Since the matrix elements inside the summation do not depend on $k$ we have that the terms with $k\neq k'$ are equal to zero. So the equation above becomes
\[
    \big\langle n,\nu,\vect k\big|H\big| n',\nu',\vect k'\big\rangle= 
...
\begin{figure}[h]
    ...
    \caption{Each one of the potentials $V(ss\sigma),V(sp\sigma),V(pp\sigma),V(pp\pi)$ represent a different kind of bonding between the two orbitals. The first two indices represent the orbitals ($s$ and $p$), the last one represent the bonding type ($\sigma$ and $\pi$). Image taken from \cite{gibertiniground}.}
...
($\sigma$-bands).\\
Fortunately the $\sigma$-bands are fully below the Fermi-level, so they do not have any contribution to the electron-transport phenomenon, so we only have to deal with the $2\times 2$ matrix of the $\pi$-bands
\begin{equation}
...
\begin{equation}
    ...
    \end{bmatrix}\,.
\end{equation}
However before diagonalizing the Hamiltonian we have to keep in mind that we do not start off with an orthonormal base of states
\begin{equation}
    \braket{p_z,\nu,\vect k}{p_z,\nu',\vect k}\equiv S_{\nu\nu'}(\vect k)=
    \begin{bmatrix}
        1 & sF(\vect k)\\
...
\begin{figure}[h]
    ...
    \caption{This is the dispersion relation for the single layer graphene, notice how near the vertices of the reciprocal lattice we get a cone-like dispersion relation typical of relativistic massless particles. Image taken from \cite{guinea2008review}.}
...
The eigenstates of this Hamiltonian are called \textbf{Dirac-fermions} and are equal To
\begin{equation}
    ...
    k\begin{bmatrix}
        ...
        \sin\theta
    \end{bmatrix}\,,
    \label{eq:monolayer_eigenstates}
\end{equation}
where $\lambda=1$ in $K_0$ and $\lambda=-1$ in $K_1$. Hence, the low-energy quasiparticles in graphene can be interpreted as massless Dirac fermion, and described by a spinor wave function in which the spin degree-of-freedom is substituted with a sublattice degree-of-freedom, that is defined as \textbf{pseudospin}.
We can also go further on this, since an electron can be in a superposition between the two cones we can add an additional spin-like degree of freedom. The Hamiltonian \ref{eq:dirac_Hamiltonian} becomes
\begin{equation}
...
\subsubsection*{Why Dirac cones appear in the first place}
Many of the properties of crystalline band structure can be infered by the symmetries of their spatial structure \cite{cardona2005fundamentals}, in this subsection we are going to talk about some of the properties of that give rise the Dirac cones.

The Hamiltonian of graphene can be written as $2\times 2$ Hamiltonian that depends from a parameter $\vect k$ though the Peierls substitution that we introduced in section \ref{sec:berrybloch}
\begin{equation}
    H(\vect k)=\vect h(\vect k)\cdot\vect \sigma\,,
\end{equation}
where $\vect h(\vect k)=(h_x(\vect k),h_z(\vect k),h_z(\vect k))$ the parity operator $\mathcal P$ sends $\vect k\to -\vect k$ and exchanges the two sites so it acts as a $\sigma_x$ operator, so we have that
\begin{equation}
    H(\vect k)=\mathcal P^{-1}H(\vect k)\mathcal P=\sigma_x H(-\vect k)\sigma_x\,,
\end{equation}
and since 
\begin{equation}
    \mathcal P^{-1}\vect \sigma\mathcal P=\sigma_x \vect \sigma\sigma_x =(\sigma_x,-\sigma_y,-\sigma_z)\,,
\end{equation}
 we have that
\begin{equation}
    \begin{vmatrix}
    ...
    \begin{vmatrix}
        ...
        -h_z(-\vect k)
    \end{vmatrix}\,.
    \label{eq:parity_cond}
\end{equation}
The time reversal operator $\mathcal T$ sends $\vect k\to -\vect k$ and it is antinunitary, so the system to have time reversal symmetry must obey 
\begin{equation}
    H(\vect k)=\mathcal T^{-1}H(\vect k)\mathcal T=H^*(\vect -k)\,,
\end{equation}
however doesn't exchange the particle sites, it only does the complex conjucation
\begin{equation}
    \mathcal T^{-1}\vect \sigma\mathcal T=(\sigma_x,-\sigma_y,\sigma_z)\,,
\end{equation}
so we have that
\begin{equation}
    \begin{vmatrix}
    ...
    \label{eq:time_cond}
\end{equation}
Uniting the conditions from equation \ref{eq:parity_cond} and equation \ref{eq:time_cond} we have that
\begin{equation}
    h_z(\vect k)=0\,.
\end{equation}
%This alone doesn't explain why there should be cone-like structures at the corners of the Brillouin zone. To prove this 
The Dirac points occur because the two component $\vect h(\vect k)$ can have point zeros in two dimensions. In graphene they occur at two points $\vect K_0$ and $\vect K_1 = -\vect K_0$ \cite{hasan2010colloquium,divincenzo1984self}.